...
export default function HomeClient() {
  ...
  const [/*isLoadingMarketplaces*/, setIsLoadingMarketplaces] = useState(true); // Uncommented this line
  ...
  const handleCategorySelect = (slug: string) => {
    ...
    if (!slug || !categoryTree) {
      ...
      const parent = Object.keys(categoryTree).find(label => label.toLowerCase().replace(/\s+/g, "-") === slug);
  ...
  const handleDropdownContainerMouseLeave = () => {
    dropdownHoverTimeoutRef.current = setTimeout(() => setIsCategoryDropdownVisible(false), DROPDOWN_CLOSE_DELAY);
  };
  const toggleSiteFilterSidebar = () => {
    setIsSiteFilterSidebarOpen(prev => !prev);
    if (isSiteFilterDropdownOpen) setIsSiteFilterDropdownOpen(false);
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const parents: Category[] = Object.keys(tree).map(name => ({ label: name, slug: name.toLowerCase().replace(/\s+/g, "-") }));
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        CORE_STYLED_SITE_DEFINITIONS.forEach(coreDef => {
          const apiMatch = fetchedMarketplacesFromAPI.find(apiSite => apiSite.site_id && apiSite.site_id.toLowerCase() === coreDef.site_id.toLowerCase());
          finalSiteList.push(apiMatch || { name: coreDef.defaultName, site_id: coreDef.site_id });
          processedSiteIds.add(coreDef.site_id.toLowerCase());
        });
        fetchedMarketplacesFromAPI.forEach(apiSite => {
          if (apiSite.site_id && !processedSiteIds.has(apiSite.site_id.toLowerCase())) {
            finalSiteList.push(apiSite);
          }
        });
        ...
        setMarketplaces([{ name: "All Sites", site_id: "" }, { name: "GET4LESSGHANA", site_id: "get4lessghana" }, { name: "Telefonika", site_id: "telefonika" }, { name: "MyGHMarket", site_id: "myghmarket" }, { name: "Shopwice", site_id: "shopwice" }].sort((a,b)=>a.name.localeCompare(b.name)));
      } finally {
  ...
  useEffect(() => {
    ...
    if (!hasCriticalChange && currentPage > 1) {
      newParams.set("page", currentPage.toString());
    }
    if (newParams.toString() !== params.toString()) {
      router.replace(`/?${newParams.toString()}`, { scroll: false });
    }
  }, [debouncedSearchTerm, selectedCategorySlug, selectedSiteId, debouncedMinPrice, debouncedMaxPrice, currentLimit, currentPage, router, params, selectedBrands, selectedTypes]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const min = debouncedMinPrice !== "" && !isNaN(+debouncedMinPrice) ? +debouncedMinPrice : undefined;
        const max = debouncedMaxPrice !== "" && !isNaN(+debouncedMaxPrice) ? +debouncedMaxPrice : undefined;
  ...
  }, [debouncedSearchTerm, expandedCategorySlugs, selectedSiteId, debouncedMinPrice, debouncedMaxPrice, currentPage, currentLimit, isLoadingApp, initialLimit, applyFilterTrigger, selectedBrands, selectedTypes]);
  ...
  }, [initialQ, initialCat, initialSiteId, initialMinPrice, initialMaxPrice]);

  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (siteFilterDropdownRef.current && !siteFilterDropdownRef.current.contains(event.target as Node)) {
        setIsSiteFilterDropdownOpen(false);
      }
      if (isCategoryModalOpen && modalContentRef.current && !modalContentRef.current.contains(event.target as Node) && moreButtonRef.current && !moreButtonRef.current.contains(event.target as Node)) {
    ...
    const handleEscapeKey = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        ...
        if (isSiteFilterSidebarOpen) setIsSiteFilterSidebarOpen(false);
      }
    };
  ...
  return (
    <main className="min-h-screen bg-gray-100 text-gray-800 flex flex-col">
      ...
      <aside
        ...
        className={`fixed top-1/2 transform -translate-y-1/2 z-[75] p-2 bg-emerald-600 rounded-r-lg shadow-lg hover:bg-emerald-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 transition-all duration-300 ease-in-out ${isSiteFilterSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <FilterSidebar
          filters={{
      ...
      <div className={`flex flex-col flex-grow transition-all duration-300 ease-in-out ${isSiteFilterSidebarOpen ? `md:ml-${SIDEBAR_WIDTH_CLASSES.split(' ')[1].substring(3)}` : ''}`}>
        ...
        <div className={`flex-grow w-full`}>
          ...
          <section className="px-4 sm:px-6 py-12 max-w-7xl mx-auto w-full">
            ...
            {!loading && !error && displayProducts.length === 0 && products.length === 0 && totalProducts === 0 && !debouncedSearchTerm && !selectedCategorySlug && !selectedSiteId && !minPrice && !maxPrice && (
              <div className="text-center py-16">
                <p className="text-xl text-gray-600 mb-4">Welcome to Qomprice!</p>
                <p className="text-gray-500 text-sm">Use the search bar or categories to find products.</p>
              </div>
            )}
            {!loading && !error && displayProducts.length === 0 && (debouncedSearchTerm || selectedCategorySlug || selectedSiteId || minPrice || maxPrice) && (
              <div className="text-center py-16">
                <p className="text-xl text-gray-600 mb-4">No products found for your current selection.</p>
                <p className="text-gray-500 text-sm">Try a different search term or adjusting your filters.</p>
              </div>
            )}
